...
describe('Test Dummy Server Example Page', () => {
    ...
    it("Update the headline and verify the change", (done) => {
        const headline = 'NEWHEAD'
        const oldline = "old headline"
        console.log(oldline)
        ...
        sleep(500)
            .then(findId('newHeadline').sendKeys(headline)
                ...
                .then(findId('message').getText()
                    .then(text => {
                        console.log(text)
                        expect(text.indexOf(headline)).to.equal(0)
                    })))
            .then(findId('newHeadline').clear())
            .then(findId('newHeadline').sendKeys(oldline)
                .then(findId("headline").click())
                .then(sleep(500))
                .then(findId('message').getText()
                    .then(text => {
                        console.log(text)
                        expect(text.indexOf(oldline)).to.equal(0)
                    })))
            .then(done)
        // IMPLEMENT ME
